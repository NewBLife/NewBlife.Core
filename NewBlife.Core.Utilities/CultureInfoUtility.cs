...
using System.Linq;
using NewBlife.Core.Extensions;

namespace NewBlife.Core.Utilities
{
    /// <summary>
    /// CultureInfo check utility class
    /// </summary>
    public static class CultureInfoUtility
    {
        ...
        /// <summary>
        /// The cultures system Implemented
        /// </summary>
        private static readonly List<CultureInfo> _cultures = new List<CultureInfo> {
            new CultureInfo("en-US"),  // default:english(us)
            new CultureInfo("zh-CN")  // chinese
        };

        /// <summary>
        /// Gets the implemented culture.
        /// when null return the default culture.
        /// </summary>
        /// <param name="name">The name.</param>
        /// <returns>culture</returns>
        public static CultureInfo GetImplementedCulture(string name = null)
        {
            ...
            if (_validCultures.Where(c => c.Equals(name, StringComparison.OrdinalIgnoreCase)).Count() == 0)
            {
                // no valiable data
                return GetDefaultCulture();
            }

            var culture = _cultures.SingleOrDefault(c => c.Name.Equals(name, StringComparison.OrdinalIgnoreCase));
            if (culture != null)
            {
                // Implemented culture
                return culture;
            ...
            culture = _cultures.SingleOrDefault(c => c.Name.StartsWith(n));
            if (culture != null)
            {
                return culture;
            }
